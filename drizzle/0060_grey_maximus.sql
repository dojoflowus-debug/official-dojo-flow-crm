...
CREATE TABLE `dojo_settings` (
	...
	`organizationId` int,
	`kioskTheme` varchar(50) DEFAULT 'default'
);
--> statement-breakpoint
...
CREATE TABLE `kai_conversations` (
	`id` int AUTO_INCREMENT NOT NULL,
	`organizationId` int NOT NULL DEFAULT 120001,
	`userId` int NOT NULL,
...
CREATE TABLE `kai_messages` (
	...
	`organizationId` int NOT NULL DEFAULT 180001,
...
CREATE TABLE `organizations` (
	...
	`settings` text,
	`onboardingStatus` enum('not_started','in_progress','completed','skipped') NOT NULL DEFAULT 'not_started',
...
ALTER TABLE `users` ADD `globalRole` enum('platform_admin','support','none') DEFAULT 'none' NOT NULL;--> statement-breakpoint
...
ALTER TABLE `users` ADD `locationIds` text;--> statement-breakpoint
...
CREATE INDEX `idx_kiosks_org_location` ON `kiosks` (`organizationId`,`locationId`);--> statement-breakpoint
CREATE INDEX `idx_kiosks_slug` ON `kiosks` (`organizationId`,`slug`);--> statement-breakpoint
...
CREATE INDEX `idx_organizationId` ON `owner_profiles` (`organizationId`);--> statement-breakpoint
...
CREATE INDEX `preset_backgrounds_key_unique` ON `preset_backgrounds` (`key`);--> statement-breakpoint
CREATE INDEX `idx_category` ON `preset_backgrounds` (`category`);--> statement-breakpoint
CREATE INDEX `idx_active` ON `preset_backgrounds` (`isActive`);--> statement-breakpoint
...
CREATE INDEX `idx_tuition_due_date` ON `student_tuition` (`dueDate`);--> statement-breakpoint
