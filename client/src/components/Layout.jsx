    ...
    try {
      if (typeof localStorage !== 'undefined') {
        ...
        if (saved) {
          ...
          // Merge saved order with default navigation to handle new items
          const merged = savedOrder.map(savedItem => 
            defaultNavigation.find(item => item.id === savedItem.id) || savedItem
          )
          // Add any new items that weren't in saved order
          const newItems = defaultNavigation.filter(
            item => !savedOrder.find(saved => saved.id === item.id)
          )
      ...
      >
        <div className="flex flex-col h-full">
          ...
          <div className="flex items-center justify-between p-4 border-b border-border">
            ...
            >
              <img src={logo} alt="Dojo Flow" className="h-8 w-8" />
