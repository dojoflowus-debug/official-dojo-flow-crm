...
## Bug Fix - TRPC Metadata Validation Error (Feb 2, 2026)
- [ ] Fix TRPC schema validation for metadata field in kai.addMessage
  - Error: "Invalid input: expected string, received object"
  - Root cause: TRPC input schema expects metadata as z.record(z.any()) but validation is failing
  - Need to check if schema definition matches actual usage


## Bug Fix - TRPC Metadata Validation Error (COMPLETED)
- [x] Fixed TRPC validation error "Invalid input: expected string, received object" on /kai page
  - Added `metadata: text()` column to messages table schema in drizzle/schema.ts
  - Executed SQL migration: `ALTER TABLE messages ADD COLUMN metadata TEXT`
  - Fixed TRPC schema in server/routers.ts (2 occurrences at lines 1311 and 2792)
  - Changed from `metadata: z.string().optional()` to `metadata: z.record(z.any()).optional()`
  - Server code already correctly serializes metadata with JSON.stringify()
  - Created comprehensive vitest tests (3/3 passing) for metadata functionality
  - Verified metadata objects are stored as JSON strings and retrieved correctly
