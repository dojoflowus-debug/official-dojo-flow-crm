...
- [x] Fix Settings modal to default to Account tab instead of blank "settings coming soon" state
...
- [x] Add intent detection (correction, identity_update, title_update, confirmation, skip, back, objection, question, unknown) to shared/onboarding
- [x] Add entity extraction (preferred name, full name, title, school name, programs) to shared/onboarding
- [x] Integrate NLU into server processStep — handle corrections/identity updates mid-flow with response priority
...
- [x] Audit all switch cases for branching/follow-up questions inside a step
- [x] Remove all "Since you teach X, one more thing..." branching patterns
- [x] Enforce: validate → save → mark complete → move to next step (no exceptions)
- [x] Each step returns exactly one kaiMessage + one nextStep (no conditional sub-questions)
- [x] Remove martial_style branching from programs step
- [x] Ensure city_state_zip is a single step (no split into city/state/zip sub-questions)
- [x] Ensure address is a single step (no sub-questions about street/unit/etc.)
...
## Phase 24: Integrate Call Analytics Router into Main TRPC (Session 17)
- [x] Examine main TRPC router structure and integration pattern
- [x] Import callAnalyticsRouter into main routers file (routers.ts line 60)
- [x] Register callAnalyticsRouter in the main appRouter (routers.ts line 676)
- [x] Add route to CallAnalyticsDashboard page in client (appRoutes.tsx line 315)
- [x] Update CallAnalyticsDashboard to use real TRPC queries (CallAnalyticsDashboard.tsx)
